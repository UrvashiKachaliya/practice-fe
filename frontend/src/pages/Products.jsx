...
function HeroSlider() {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={prev}
        className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center transition z-20"
      >
        <FaChevronLeft size={14} />
      </button>
      <button
        onClick={next}
        className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center transition z-20">
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 pt-8">
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${offer.bg_from}, ${offer.bg_to})`,
        }}
      >
        ...
        {offers.length > 1 && (
          <>
            <button
              onClick={prev}
              className="absolute left-3 top-1/2 -translate-y-1/2 w-7 h-7 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center transition"
            >
              <FaChevronLeft size={11} />
            </button>
            <button
              onClick={next}
              className="absolute right-3 top-1/2 -translate-y-1/2 w-7 h-7 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center transition"
            >
      ...
      <section className="max-w-6xl mx-auto px-4 py-10">
          ...
          <div>
            {searchQuery ? (
              ...
              <>
                ...
                <p className="text-gray-400 text-sm mt-0.5">
                  Fresh batches, crispy every time
                </p>
              </>
            )}
          </div>
            ...
            {limitedProducts.map((product) => (
              ...
              >
                <div className="relative overflow-hidden">
                  <img
                    src={
                      product.image || "https://placehold.co/300x200?text=🌾"
                    }
                    alt={product.title}
        ...
        {filtered.length > 4 && (
          <div className="flex justify-end mt-8">
            <Link
              to="/products"
              className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 rounded-xl font-bold text-sm transition shadow-md"
            >
              Browse All Flavors →
            </Link>
          </div>
        ...
        <div className="bg-gradient-to-r from-orange-500 to-amber-500 rounded-3xl p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 text-white">
          <div>
            ...
            <p className="text-orange-100">
              Special pricing for orders above 5kg. Perfect for gifting &
              events.
            </p>
          </div>
          <Link
            to="/contact"
            className="shrink-0 bg-white text-orange-500 px-8 py-3 rounded-xl font-bold text-sm hover:bg-orange-50 shadow-lg transition whitespace-nowrap"
          >
            Contact Us →
          </Link>
        </div>
